...
# =================== APP CONFIG ===================
APP_NAME = "Mate Desktop Search Ultimate"
APP_VERSION = "3.0.0"
...
def index_folder(folder):
    status_text.set("Indexing...")
    threading.Thread(target=_index_worker,args=(folder,),daemon=True).start()

def _index_worker(folder):
    indexed_files.clear()
...
def windows_search(query):
    cmd = f'''
    Get-ChildItem -Path "{current_folder.get()}" -Recurse |
...
def update_results(*args):
    ...
    if sys.platform.startswith("win") and current_folder.get():
        matches = windows_search(q)
    else:
        matches = []
    ...
    if not matches:
        for f in indexed_files:
            ...
            name = q in os.path.basename(f).lower()
            content = q and search_content(f,q)

            if not q or name or content:
                matches.append(f)
...
def show_about():
    messagebox.showinfo(
        ...
        "Professional desktop search engine.\n\n"
        "• Background indexing\n"
